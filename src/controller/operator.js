// import OperatorWorkLog from "../models/operator.model.js";
// import operatorOilReport from "../models/operatorOilReport.js";


//  const addOperatorWorkLog = async (req, res) => {
//     const { date, shift, dredger, forward, swing, depth, meReading, aeReading, dgReading, entries } = req.body;

//     if (!date || !shift || !dredger || !forward || !swing || !depth || !meReading || !aeReading || !dgReading || !entries) {
//       return res.status(400).json({ error: 'All fields are required.' });
//     }

//     // console.log(date, shift, dredger, forward, swing, depth, meReading, aeReading, dgReading, entries)

//     try {
//       // Check if an operator work log exists for the given date and shift
//       const existingLog = await OperatorWorkLog.findOne({ date, shift });

//       if (existingLog) {
//         // Replace existing record
//         existingLog.dredger = dredger;
//         existingLog.forward = forward;
//         existingLog.swing = swing;
//         existingLog.depth = depth;
//         existingLog.meReading = meReading;
//         existingLog.aeReading = aeReading;
//         existingLog.dgReading = dgReading;
//         existingLog.entries = entries;

//         await existingLog.save();
//         return res.json({ message: 'Operator work log updated successfully.', operatorWorkLog: existingLog });
//       } else {
//         // Create new record
//         const newLog = new OperatorWorkLog({
//           date,
//           shift,
//           dredger,
//           forward,
...
//         await newLog.save();
//         return res.json({ message: 'Operator work log created successfully.', operatorWorkLog: newLog });
//       }
//     } catch (error) {
//       console.error('Error saving operator work log:', error);
//       res.status(500).json({ error: 'Internal server error.' });
//     }
//   };

//   const addOilReportByOperator = async (req, res) => {
//     try {
...
//       // Check if a report already exists for the given date and shift
//       const existingReport = await operatorOilReport.findOne({ date, shift });

//       if (existingReport) {
//         // Update the existing report
//         existingReport.data = data;
//         await existingReport.save();
//         return res.status(200).json({ message: "Oil report updated successfully." });
//       }

//       // Create a new report
//       const newReport = new operatorOilReport({ date, shift, data });
//       await newReport.save();

//       res.status(201).json({ message: "Oil report created successfully." });
//     } catch (error) {
//       console.error("Error saving oil report:", error);
//       res.status(500).json({ error: "Internal server error." });
//     }
//   };

//   const getOilReportByDate = async (req, res) => {
//     try {
...
//       // Fetch data for the given date and shift
//       const oilReport = await operatorOilReport.findOne({ date, shift });

//       // Initialize with default OB if no data found
//       if (!oilReport) {
//         const defaultData = [
//           { name: "H.S.D", OB: 100, balance: 100 },
//           { name: "15 W 40", OB: 150, balance: 150 },
//           { name: "H.Y.D.68", OB: 200, balance: 200 },
//           { name: "Grease", OB: 50, balance: 50 },
//           { name: "Coolant", OB: 75, balance: 75 },
//           { name: "Compound", OB: 120, balance: 120 },
//         ];

//         return res.status(200).json(defaultData);
//       }

//       res.status(200).json(oilReport.data);
//     } catch (error) {
